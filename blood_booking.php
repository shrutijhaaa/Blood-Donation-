...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($name) || empty($email) || empty($phone_number) || empty($blood_group) || empty($state) || empty($date) || empty($time)) {
            ...
            $stmt_check->bind_param("ss", $email, $time);
            $stmt_check->execute();
            ...
            if ($result_check->num_rows > 0) {
                ...
                while ($row = $result_check->fetch_assoc()) {
                    if ($row['Email'] === $email || $row['Time'] === $time) {
                        $duplicate = true;
                        break;
                    }
                }
                if ($duplicate) {
                    $message = 'Error: A booking of same user already exists.';
                    $message_type = 'error';
                }
            } else {
                ...
                try {
                    if ($stmt->execute()) {
                        ...
                        $message = 'Error: Could not insert record.';
                        $message_type = 'error';
                    }
                } catch (mysqli_sql_exception $e) {
                    if ($e->getCode() == 1062) { // 1062 is the error code for duplicate entry
                        $message = 'Duplicate entry! Please check your data.';
                        $message_type = 'error';
                    } else {
